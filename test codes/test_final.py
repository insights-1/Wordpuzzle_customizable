...
bg_image = Image.open("background.jpg").convert("RGBA")
...
for i in range(len(crossword)):
    ...
    for j in range(len(crossword[i])):
        ...
        btn.grid(row=i, column=j, padx=5, pady=5, ipadx=5, ipady=5, sticky="nsew")
...
for idx, word in enumerate(correct_words):
    lbl = tk.Label(root, text=word, font="Arial 10 bold", bg="lightgray")
...
def resize_grid(event):
    global button_size
    # Calculate new button size based on the new window size
    new_size = min(event.width, event.height) // len(crossword)
    button_size = max(new_size, 20)
