...
# Create a vertical list of words beside the grid
names_frame = tk.Frame(root, bg="lightgray")
names_frame.grid(row=0, column=1, sticky="ns")

word_labels = []
for idx, word in enumerate(correct_words):
    lbl = tk.Label(names_frame, text=word, font="Arial 10 bold", bg="lightgray")
    lbl.grid(row=idx, column=0, padx=10, pady=5, sticky="w")
    word_labels.append(lbl)

# Empty Space Frame
empty_space = tk.Frame(root, bg="peachpuff")
empty_space.grid(row=0, column=2, sticky="nsew")
