...
def check_word(selected_buttons, correct_words, word_labels, image_label, image_paths):
    ...
    if selected_word in correct_words_lower:
        ...
        found_word = correct_words[correct_word_index]
        update_word_list(found_word, word_labels)
        correct_words.pop(correct_word_index)

        # Display the image for the found word
        if found_word in image_paths:
            img = Image.open(image_paths[found_word])
            img = img.resize((300, 300), Image.ANTIALIAS)  # Resize image if needed
...
def handle_button_event(event, selected_buttons, button_grid, correct_words, word_labels, image_label, image_paths):
    if event.type == tk.EventType.ButtonPress:
        ...
        check_word(selected_buttons, correct_words, word_labels, image_label, image_paths)
...
# # Paths to images corresponding to each correct word
# image_paths = {
#     "HenryFord": "C:/Users/vivek/Desktop/NEW/henry_ford.png",
#     # "CharlesBabbage": "charles_babbage.jpg",
#     # "EdwardsDeming": "edwards_deming.jpg",
#     # # Add paths for other words similarly
# }
...
for i in range(len(crossword)):
    ...
    for j in range(len(crossword[i])):
        ...
        btn.bind("<ButtonPress-1>", lambda event: handle_button_event(event, selected_buttons, button_grid, correct_words, word_labels, image_label, image_paths))
        btn.bind("<B1-Motion>", lambda event: handle_button_event(event, selected_buttons, button_grid, correct_words, word_labels, image_label, image_paths))
        btn.bind("<ButtonRelease-1>", lambda event: handle_button_event(event, selected_buttons, button_grid, correct_words, word_labels, image_label, image_paths))
        row.append(btn)
...
# # Create a label to display images when a correct word is found
# image_label = tk.Label(root, bg="white")
# image_label.grid(row=0, column=1, padx=210, pady=260, sticky="e")
...
# scientist_images = {
#     "HenryFord": ImageTk.PhotoImage(file="C:/Users/vivek/Desktop/new/henry_ford.png"),
